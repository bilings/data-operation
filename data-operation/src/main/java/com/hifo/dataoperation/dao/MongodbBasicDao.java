...
public abstract class MongodbBasicDao<T> {
    ...
    public void updateById(String id, T info) {
        Document document = (Document) mongoConverter.convertToMongoType(info);
        mongoTemplate.getCollection(this.getCollectionName()).updateOne(new Document("_id",new ObjectId(id)),new Document("$set",document));
    }
	 ...
	 */
	public List<T> groupFind(AggregationOperation... operations) {
		Aggregation agg = Aggregation.newAggregation(operations);
		AggregationResults<T> results = this.mongoTemplate.aggregate(agg, this.getCollectionName(),
				this.getEntityClass());
    ...
    public Long countById() {
    	return mongoTemplate.count(new Query(Criteria.where("_id").exists(true)),this.getEntityClass());
    }
