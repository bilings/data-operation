...
public class BuildingController extends BasicController {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "buildingType", value = "建筑类别", paramType = "query", dataType = "String"),
            @ApiImplicitParam(name = "isDeleted", value = "是否删除", paramType = "query", dataType = "Integer"),
            @ApiImplicitParam(name = "visibility", value = "是否可见", paramType = "query", dataType = "String"),
    ...
    @ResponseBody
    @RequiredPermission("100")
    public ApiResult<BusBuilding> findById(String id) {
    ...
    @GetMapping("/init")
    @RequiredPermission("100")
    public ApiResult<String> init(String communityId,String type) {
        // 分页数据
        String s = buildingService.init();
        // 保存查询条件用于导出
    ...
    public ApiResult<Map> log(String buildingId,String type) {
        // 分页数据
        ApiResult<Map> api = buildingService.log(buildingId,type);
        // 保存查询条件用于导出
        return api;
    }
    ...
    @ApiImplicitParam(name = "id", value = "楼栋ID", paramType = "query", dataType = "String", example = "1", required = true)
    @GetMapping("/mergeHistory")
    @RequiredPermission("100")
    public ApiResult<List<Document>> mergeHistory(String id) {
        return buildingService.getMergeListByToId(id);
    }
