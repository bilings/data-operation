var estateEdit_obj = {
    ...
    _initIcon: function () {
        var _this = this;
        $('#estateEdit_wrapper #nameIcon').on('click', function () {
            layer.open({
                type: 2,
                title: '数据来源',
                content: 'log.html?communityId='+_this.id+"&type=nameIcon",
                area: ['97%', '97%']
            });
        });
        $('#estateEdit_wrapper #blockIcon').on('click', function () {
            layer.open({
                type: 2,
                title: '数据来源',
                content: 'log.html?communityId='+_this.id+"&type=blockIcon",
                area: ['97%', '97%']
            });
        });
        $('#estateEdit_wrapper #roadIcon').on('click', function () {
            layer.open({
                type: 2,
                title: '数据来源',
                content: 'log.html?communityId='+_this.id+"&type=roadIcon",
                area: ['97%', '97%']
            });
        });
        $('#estateEdit_wrapper #numberIcon').on('click', function () {
            layer.open({
                type: 2,
                title: '数据来源',
                content: 'log.html?communityId='+_this.id+"&type=numberIcon",
                area: ['97%', '97%']
            });
        });
    },
        ...
        $.get({
            ...
            success: function (res) {
                ...
                try {
                    ...
                    // 详细地址是json数组，特殊
                    $('#estateEdit_wrapper #road').val(_this.estate.stdAddress.road);
                    $('#estateEdit_wrapper #number').val(_this.estate.stdAddress.number);
                } catch (e) {
                }
    ...
    _initCrumbs: function () {
        ...
        $('#estateEdit_wrapper #buildingList').on('click', function () {
            parent.homeIndex_obj._addTab('楼栋-' + _this.estate.name, '../building/index.html?communityId=' + _this.estate.id, false, '');
        });
        // 合并历史
    ...
    _initPositionBtn: function () {
        ...
        var location;
        try {
             location=_this.estate.location;
        }catch (e) {
        }
    ...
    _initButton: function () {
        ...
        layui.form.on('submit(updateBtn)', function (param) {
            ...
            var stdAddress = {
                ...
                address : $('#estateEdit_wrapper #road').val()+$('#estateEdit_wrapper #number').val(),
                districtId : $('#estateEdit_wrapper #districtId').val(),
                stdMethod : "web",
                city :$('#estateEdit_wrapper #cityName').val(),
                parseCode : 4,
                district : param.form.districtId.selectedOptions[0].text,
                stdTime : "2020-03-16 16:23:11",
                parseInfo : "",
            ...
            var data = {
                ...
                address:$('#estateEdit_wrapper #road').val()+$('#estateEdit_wrapper #number').val()+"号",
    ...
    _initAdvanceCheckbox: function (url, data, id) {
        ...
        $.get({
            ...
            success: function (res) {
                ...
                $.each(res.data, function (index, item) {
                    var checkbox;
                    if (_this.estate && _this.estate[id] && _this.estate[id].split(',').indexOf(item.item + '') > -1) {
                        checkbox = '<input type="checkbox" name="' + id + '" value="' + item.item + '" lay-skin="primary" title="' + item.item + '" checked=""">';
                    } else {
                        checkbox = '<input type="checkbox" name="' + id + '" value="' + item.item + '" lay-skin="primary" title="' + item.item + '">';
                    }
