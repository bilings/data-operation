...
public class EmployeeController extends BasicController {
    ...
    public ApiResult cityList() {
        return ApiResult.success(employeeService.getCityListByOrganization(basicService.getLoginEmployee().getId().toString()));
    }
    ...
    @GetMapping("/findById")
    @RequiredPermission("606")
    public ApiResult<Map> findById(String id) {
        Map employee = employeeService.findById(id);
        // 避免前端undefined异常
        if (employee == null) {
